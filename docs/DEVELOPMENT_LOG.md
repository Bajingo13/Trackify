...
| End-to-end CI job | Added an advisory workflow job that migrates, seeds demo data, starts the API, and runs the suites. | Advisory on purpose. On a freshly migrated database `odometer` exits 2 — there is no cached route geometry, because that is backfilled from OSRM by a script rather than a migration. 7 of 8 pass on CI; drop `continue-on-error` once the routes are seeded. |
