...
export default function MaintenancePage() {
  ...
  const loadData = () => {
    setData(getAllMaintenance({ search, status: statusFilter, page, limit: 10 }));
    setStats(getMaintenanceStats());
  };
  useEffect(() => { loadData(); }, [search, statusFilter, page]);

  const handleCreate = (f) => { createMaintenance(f); addToast("Maintenance scheduled"); setView("list"); loadData(); };
  const handleUpdate = (f) => { updateMaintenance(editingRecord.id, f); addToast("Maintenance updated"); setEditingRecord(null); loadData(); };
  const handleComplete = () => {
    const r = completeMaintenance(completingRecord.id, completeForm);
    if (r.success) { addToast("Maintenance marked as completed"); setCompletingRecord(null); setCompleteForm({ cost: "", partsUsed: "", findings: "" }); loadData(); }
  };
  const handleDelete = () => {
    const r = deleteMaintenance(deletingRecord.id);
    if (r.success) { addToast("Maintenance record deleted"); } else { addToast(r.message, "error"); }
