...
export default function VehiclesPage() {
  ...
  const loadData = async () => {
    try {
      setData(await getAllVehicles({ search, status: statusFilter, type: typeFilter, page, limit: 10 }));
  ...
  const handleCreate = async (d) => {
    try { await createVehicle(d); addToast("Vehicle added"); setView("list"); loadData(); }
    catch (e) { addToast(e.message || "Failed to add vehicle", "error"); }
  };
  const handleUpdate = async (d) => {
    try { await updateVehicle(editingVehicle.id, d); addToast("Vehicle updated"); setEditingVehicle(null); loadData(); }
    catch (e) { addToast(e.message || "Update failed", "error"); }
  };
  const handleOdometerSave = async (id, reading) => {
    const r = await updateOdometer(id, reading);
    if (r.success) { addToast("Odometer updated"); setOdometerVehicle(null); loadData(); } else { addToast(r.message, "error"); }
  };
  const handleDelete = async () => {
    const r = await deleteVehicle(deletingVehicle.id);
    if (r.success) { addToast("Vehicle retired"); } else { addToast(r.message, "error"); }
